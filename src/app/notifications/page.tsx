        ...
        <div className="px-8 py-6 max-w-3xl">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-muted-foreground text-sm mt-1">You will see alerts here when invoices are paid and more.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {notifications.map((n) => (
                <div key={n.id} className={"bg-card border rounded-xl p-4 flex items-start gap-4 transition " + (n.read ? "border-border opacity-60" : "border-primary/30")}>
                  <div className={"w-9 h-9 rounded-full flex items-center justify-center text-lg shrink-0 " + (typeStyles[n.type] || typeStyles.info)}>
                    {typeIcons[n.type] || "💬"}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <button onClick={() => deleteNotification(n.id)} className="text-xs text-destructive hover:underline flex items-center gap-1">
