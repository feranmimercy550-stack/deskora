...
import {
  ...
  Settings, Quote, TrendingUp, TrendingDown
} from "lucide-react";
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="px-8 py-4 border-b border-border bg-card">
          ...
          <p className="text-muted-foreground text-sm">Insights about your business performance.</p>
        </div>
        ...
        <div className="px-8 py-6 space-y-6">
          {loading ? (
            ...
            <>
              ...
              {/* Invoice Breakdown + Top Customers */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-card border border-border rounded-xl p-6">
                  ...
                  <div className="space-y-4">
                    {[
                      { label: "Paid", value: data.paidInvoices, color: "bg-green-500", total: data.totalInvoices },
                      { label: "Unpaid", value: data.unpaidInvoices, color: "bg-yellow-500", total: data.totalInvoices },
                      { label: "Overdue", value: data.overdueInvoices, color: "bg-red-500", total: data.totalInvoices },
                    ].map((item) => (
                      <div key={item.label}>
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-xl p-6">
                ...
                <p className="text-muted-foreground text-sm leading-relaxed">
                  ...
                  <strong className="text-green-600">{data.paidInvoices} invoices are paid</strong>,{" "}
                  {data.unpaidInvoices > 0 && <><strong className="text-yellow-600">{data.unpaidInvoices} are unpaid</strong>, </>}
                  {data.overdueInvoices > 0 && <><strong className="text-red-600">{data.overdueInvoices} are overdue</strong>. </>}
                  Your total revenue collected is{" "}
                  <strong className="text-foreground">₦{data.totalRevenue.toLocaleString()}</strong>.
                </p>
              </div>
            </>
          )}
