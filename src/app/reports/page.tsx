...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      setData({ totalRevenue, totalInvoices: invoices.length, paidInvoices, unpaidInvoices, overdueInvoices, totalCustomers: customers.length, totalPayments: payments.length, topCustomers });
      setLoading(false);
    };
    fetchReports();
  }, []);
                      ...
                      <div key={item.label}>
                        ...
                        <div className="w-full bg-muted rounded-full h-2">
                          <div className={`${item.color} h-2 rounded-full`}
                            style={{ width: data.totalInvoices > 0 ? `${(item.value / data.totalInvoices) * 100}%` : "0%" }}></div>
                        </div>
                      </div>
                    ))}
                  </div>
